...
def prepare_parser():
  ...
  parser.add_argument("--data_folder", type=str)       # 数据集所处位置
  parser.add_argument("--warmup_epochs", type = float, default = 200)  # 一致性正则化的预热epoch

  parser.add_argument("--id",type=str, default="")     # 保存结果的文件夹名称
  parser.add_argument("--gpus", type=str, default="")  # 使用的gpu
  parser.add_argument("--sample_every", type=int,default=1000)  # 每1000个iter保存一次sample
  parser.add_argument("--resume_from", type = str)
  parser.add_argument("--epoch_id", type=str, default="")
  parser.add_argument("--unconditional", action="store_true", default = False)  # 是否使用标签帮助训练(cgan思想)

  parser.add_argument("--slow_mixup", action="store_true", default = False)     # Use warmup for the CutMix-augmentation loss.
  parser.add_argument("--consistency_loss", action="store_true", default = False)  # Compute only the CutMix consistency loss, but not the CutMix augmentation loss
  parser.add_argument("--consistency_loss_and_augmentation", action="store_true", default = False)  # 使用CutMix-augmentation loss
  parser.add_argument("--full_batch_mixup", action="store_true", default = False)

  parser.add_argument("--debug", action='store_true',default=False)
  parser.add_argument("--dataloader", type=str, default="celeba128")  # 数据集名称
  parser.add_argument("--unet_mixup", action="store_true", default = False)  # Use CutMix
  ...
  parser.add_argument(
    ...
    help='How 2 train your bois (default: %(default)s)')
...
# 正交规则化（orthogonal regulatization） -> 能够稳定训练,缓解梯度爆炸和梯度消散情况的发生
# 这个是改进版，default_ortho是默认版
...
# 计算1000个iters所花的时间，而不是完成全部epoch所花的时间
"""
...
def sample(G, z_, y_, config):
  with torch.no_grad():
    z_.sample_()
    y_.sample_()
    # nn.parallel.data_parallel()表示多GPU使用
    if config['parallel']:
